...
void CameraSys::setPlayerID(ID id) { playerID = id; }


glm::vec3 CameraSys::getForwardVec() { return fwd; }
glm::vec3 CameraSys::getLeftVec() { return left; }
glm::vec3 CameraSys::getUpVec() { return up; }
...
glm::mat4 CameraSys::updateView() {
	...
	//printf("camera fwd: x %f y %f z %f\n", fwd.x, fwd.y, fwd.z);

	//if (glm::length(fwd) == 0.0f) { fwd = glm::vec3(1.0f, 0.0f, 0.0f); }
	//note on the above check:
	//it is very likely that the first draw() call for world will happen before the first update() call
	//therefore, the quaternion for the player may not be fully initialized
