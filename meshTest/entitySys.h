...
enum componentEnum {PHYSICS_CMP, RENDER_CMP, NUMBER_OF_CMPS};
...
public:
	...
	template <class T> void addPrefabCmp(ID prefab_id, T& cmp);
	template <class T> void removePrefabCmp(ID prefab_id);
	
	//these 4 functions must be specialized for each component
	template <class T> ID_LOOKUP_TABLE<T>& getTable();
	template <class T> ID   getExposedID(ID internalID);
	template <class T> ID   getInternalID(ID exposedID);
	template <class T> void setInternalID(ID exposedID, ID internalID);

	//defined in header file (no specialization needed)
	template <class T> bool has(ID exposedID);
	template <class T> T& get(ID exposedID);
	template <class T> T* getAll();
	template <class T> unsigned int getSize();
	template <class T> ID add(ID exposedID, T& cmp);
	template <class T> void remove(ID exposedID);
