...
int FiniteStateMachine::getWidth() { return width; }
int FiniteStateMachine::getHeight() { return height; }
...
bool FiniteStateMachine::update(float t, const float dt) {
	...
	if (inputRes.keys[KEY_EXIT]) {return resolveReturnedState(RETURN);}
...
bool FiniteStateMachine::resolveReturnedState(gameState state) {
	...
	if (stateStack.front() == GAME) { window.setMouseCursorVisible(false); } else { window.setMouseCursorVisible(true); }

	return true;

}

void FiniteStateMachine::keyPressed(sf::Keyboard::Key key) {input.keyPressed(key);}
void FiniteStateMachine::keyReleased(sf::Keyboard::Key key) {input.keyReleased(key);}
void FiniteStateMachine::mouseUpdate(sf::Vector2i mousePosition) {input.mouseUpdate(mousePosition);}