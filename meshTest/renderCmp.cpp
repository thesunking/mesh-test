...
RenderCmp::RenderCmp() {
	...
	scalingMatrix	  = glm::scale(1.0f, 1.0f, 1.0f);
...
RenderCmp::RenderCmp(float* vrt, unsigned int* ind) {
	...
	scalingMatrix	  = glm::scale(10.0f, 10.0f, 10.0f);
	rotationMatrix    = glm::rotate( 0.0f, glm::vec3(1.0f, 0.0f, 0.0f) );
	translationMatrix = glm::translate(0.0f, 0.0f, 0.0f);
...
void RenderCmp::updateScalingMatrix(glm::vec3 scalar) { scalingMatrix = glm::scale(scalar.x, scalar.y, scalar.z); }
