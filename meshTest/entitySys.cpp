...
ID EntitySys::createEntity(ID prefab_id) {
	...
	if (prefab.cmpList[PHYSICS_CMP]) { entity.cmpIDs[PHYSICS_CMP] = physicsComponents.add(prefab.phyCmp); }
	if (prefab.cmpList[RENDER_CMP]) { entity.cmpIDs[RENDER_CMP] = renderComponents.add(prefab.rndCmp); }
...
template <> void EntitySys::addPrefabCmp<PhysicsCmp>(ID prefab_id, PhysicsCmp& cmp) {
	PrefabContainer& prefab = prefabs.get(prefab_id);
	prefab.phyCmp = cmp;
	prefab.cmpList.set(PHYSICS_CMP);
}
template <> void EntitySys::addPrefabCmp<RenderCmp>(ID prefab_id, RenderCmp& cmp) {
	PrefabContainer& prefab = prefabs.get(prefab_id);
	prefab.rndCmp = cmp;
...
template <> void EntitySys::removePrefabCmp<PhysicsCmp>(ID prefab_id) {
	PrefabContainer& prefab = prefabs.get(prefab_id);
	prefab.cmpList.reset(PHYSICS_CMP);
}
template <> void EntitySys::removePrefabCmp<RenderCmp>(ID prefab_id) {
	PrefabContainer& prefab = prefabs.get(prefab_id);
	prefab.cmpList.reset(RENDER_CMP);
}


template <> ID_LOOKUP_TABLE<CmpIDContainer>& EntitySys::getTable() {
	return aggregateComponents;
}
template <> ID_LOOKUP_TABLE<PhysicsCmp>& EntitySys::getTable() {
	return physicsComponents;
}
template <> ID_LOOKUP_TABLE<RenderCmp>& EntitySys::getTable() {
	return renderComponents;
}


template <> ID EntitySys::getExposedID<PhysicsCmp>(ID internalID) {
	...
	for (unsigned int i = 0; i != size; ++i) {
		...
		if ( cmpID.index == internalID.index && cmpID.verif == internalID.verif ) { return cmps[i].id; }
...
template <> ID EntitySys::getExposedID<RenderCmp>(ID internalID) {
...
template <> ID EntitySys::getInternalID<PhysicsCmp>(ID exposedID) {
	return aggregateComponents.get(exposedID).cmpIDs[PHYSICS_CMP];
}
template <> ID EntitySys::getInternalID<RenderCmp>(ID exposedID) {
	return aggregateComponents.get(exposedID).cmpIDs[RENDER_CMP];
}


template <> void EntitySys::setInternalID<PhysicsCmp>(ID exposedID, ID internalID) {
	aggregateComponents.get(exposedID).cmpIDs[PHYSICS_CMP] = internalID;
}
template <> void EntitySys::setInternalID<RenderCmp>(ID exposedID, ID internalID) {
