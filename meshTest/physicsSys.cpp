...
float PhysicsSys::findTimeOfCollision(Contact c, float dt)
{
	...
	for (unsigned int m = 2; m <= max_iter; m++)
	{
		...
		if (ground(p1) > 0.0f)
		{
			bisect -= powf(0.5f, (float)m);
		} //if penetrating still, decrease bisect
		else
		{
			bisect += powf(0.5f, (float)m);
		}
	...
	if (ground(p1) <= 0.0f)
	{
		bisect += powf(0.5f, (float)max_iter);
	} //if not penetrating, increase bisect to make sure we are
	p1 = p0 + rel_v*bisect*dt;
	if (ground(p1) <= 0.0f)
	{
		printf("c_focus is no longer penetrating!\n");
	}
...
void PhysicsSys::resolve_object(PhysicsCmp& phyCmp, RenderCmp &rndCmp, float t, const float dt, unsigned int iter)
{
	...
	for each (Contact c in contacts)
	{
		if (getContactType(c) == 1)
		{
			allContactsAreResting = false;
		}
	}
